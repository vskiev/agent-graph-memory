...
1. **Auto-detect changed files** — run: `git -C $(git rev-parse --show-toplevel 2>/dev/null || pwd) diff --name-only HEAD`
   Use the output as the file list. If the command fails or returns nothing, fall back to listing files mentioned in this conversation.

2. Call `update_session("claude", ["<file1>", "<file2>", ...], "$ARGUMENTS")`
